...
var restDrag = 0.4;
var physics = new ParticleSystem(+44.34, -32, 10, restDrag);
...
var blinkTween1 = new TWEEN.Tween(eyeHeight).to({ s: 5 }, 90).easing(TWEEN.Easing.Exponential.EaseIn);
var blinkTween2 = new TWEEN.Tween(eyeHeight).to({ s: 5 }, 140).easing(TWEEN.Easing.Exponential.EaseIn);
var blinkTween3 = new TWEEN.Tween(eyeHeight).to({ s: 5 }, 600).easing(TWEEN.Easing.Exponential.EaseIn);
var blinkTween4 = new TWEEN.Tween(eyeHeight).to({ s: 5   }, 180).easing(TWEEN.Easing.Exponential.EaseIn);

blinkTween1.chain(blinkTween2);
blinkTween2.chain(blinkTween3);
blinkTween3.chain(blinkTween4);
blinkTween4.chain(blinkTween1);
blinkTween1.delay(1000);
blinkTween1.start();
...
setInterval(function() {
  ...
  physics.gravity.y =  Math.sin(Date.now()/6000)*0.9 - 3
  updateAppearance();
  setPositions();
  physics.tick(2.6);
}, 1000/60);
...
function onDraw(ctx) {
  ...
  var shake = (Math.random()-Math.random()) * 100;
  ...
  ctx.rotate(angle);
  ctx.scale(peaking ? 4.2 : 1, peaking ? 1.2 : eyeHeight.s);
  circle(ctx, radius);
  ctx.restore();
  ctx.shadowBlur = 0;
  ctx.shadowOffsetY = 0;
...
function buildBody() {
  for (i = 0; i < numPoints; i++) {
    ...
    var particle = physics.makeParticle(4.5, x, y, 0);
